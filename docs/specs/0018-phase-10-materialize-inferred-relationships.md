...
The server uses a versioned canonical encoding and collision-resistant digest. Ordering, labels, timestamps, and source display names do not affect the ID.
...
For every supported fact, Kotlin computes candidates in this order:

1. writable local sources that declare the subject;
2. if none declare the subject, all writable local ontology sources in which the existing typed edit can validly assert the statement.
...
Stageability and proposal review show `Depends on imported knowledge`. Provenance records the sorted contributing import source IDs or imported ontology IRIs available from the reasoning/import boundary. Phase 10 does not promise a minimal proof or copy imported axioms into local sources.
...
The batch request uses one idempotency key. Replaying the identical completed request returns the same staging response. Reusing its key with different fact IDs or source choices fails.
...
No product-scope question blocks ExecPlan drafting. The following implementation details must be verified during ordered implementation without expanding this spec:

- whether retained import/explanation metadata is sufficient to mark every safe import-derived candidate, or whether a narrow additive reasoning-result field is required;
- whether candidate pagination is necessary after measuring the current 100-fact details limit;
- the exact canonical encoding version and digest already available in repository utilities;
- the smallest additive change that lets `StagingWorkflowService` append a prepared typed batch atomically while preserving existing idempotency behavior;
- the exact UI component split inside the currently compact `SemanticJobPanel`.

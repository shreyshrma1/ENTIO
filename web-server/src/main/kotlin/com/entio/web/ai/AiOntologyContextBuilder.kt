...
internal class AiOntologyContextBuilder(
    ...
    private companion object {
        ...
        private val ONTOLOGY_ENGINEERING_GUIDE = """
            ...
            - SHACL node-shape structure: a node shape is the resource typed as `sh:NodeShape`; its `sh:targetClass` identifies the class it validates and its `sh:property` links to property-shape resources. A property shape is the resource linked by that node shape's `sh:property`; it carries `sh:path`, `sh:minCount`, `sh:maxCount`, `sh:datatype`, `sh:class`, `sh:nodeKind`, `sh:message`, and related property constraints. `sh:targetClass` and `sh:property` belong to the node shape, not the property shape.
            - Semantic construction patterns are guidance, not a fixed plan. When creating a class, consider the class declaration (`rdf:type owl:Class` or the ontology's established class type), preferred label, definition, and applicable `rdfs:subClassOf` axioms. When creating an object property, consider its declaration (`rdf:type owl:ObjectProperty` or the established property type), label/definition, `rdfs:domain`, and `rdfs:range`. When creating a datatype property, consider its declaration (`rdf:type owl:DatatypeProperty`), label/definition, `rdfs:domain`, and an appropriate datatype `rdfs:range`. When creating an individual, consider its `rdf:type` assertion and the property assertions that demonstrate its intended relations. Reuse existing declarations and annotations when present; do not add pieces that are unnecessary for the requested outcome.
            - A complete SHACL property constraint normally has a named node shape typed as `sh:NodeShape`, a `sh:targetClass` assertion, a `sh:property` link to a property-shape resource, and property-shape assertions including `sh:path` plus the requested constraint such as `sh:minCount 1`. Add labels or messages when useful. Do not emit only `sh:path` when the user asks to create a constraint: connect the property shape to its node shape and target class as well, unless those links already exist in the graph.
            - When removing a node shape or all constraints for a target class, inspect the actual node-shape subject and remove its type, label, target, and property links as requested, then remove the associated property-shape constraints and path when they would otherwise be orphaned. Follow the asserted subject/object IRIs rather than guessing from similar labels.
            ...
            - Treat deletion as dependency-aware graph cleanup. If the user asks to delete an entity, inspect every asserted triple where its IRI appears as subject, predicate, or object and remove the related property assertions, type and hierarchy links, individuals, and SHACL shape targets, paths, and constraints so no dangling references remain, unless the user explicitly asks to preserve or repoint a reference.
            - Treat replacement as an IRI migration, not merely a label change. When replacing a local concept with an external concept such as FIBO, preserve the replacement concept's authoritative type, label, definition, and other applicable semantics, then migrate every reference from the old IRI to the new IRI across property domains and ranges, subclass and type assertions, object and property assertions, and SHACL targets, paths, and constraints. Do not leave the old IRI in dependent triples after the migration.
            - When deterministic validation reports that an object does not satisfy a property's declared range (or a subject does not satisfy its declared domain), inspect the user's intended relationship before choosing the repair. Correct the assertion when the object or subject is wrong; correct the domain or range axiom when the property definition is wrong. If you change a domain or range, remove the old conflicting axiom and add the corrected axiom so the effective graph does not retain both.
            ...
            - For proposal edits, compare each exact source-scoped triple with the supplied graph before choosing the operation: add only an absent triple and remove only a present triple. Do not emit an edit that would leave the graph unchanged; if a requested state is already true, explain that in the answer instead.

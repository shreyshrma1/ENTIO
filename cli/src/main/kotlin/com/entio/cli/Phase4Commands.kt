...
private fun shapeJson(value: ShaclNodeShape): JsonFragment = jsonObject("iri" to value.id.iri.value, "label" to value.label, "sourceId" to value.id.sourceId, "targets" to jsonArray(value.targets.map(::targetJson)), "propertyShapes" to jsonArray(value.propertyShapes.map(::propertyShapeJson)), "constraints" to jsonArray(value.constraints.map { jsonObject("kind" to it.kind.name.lowercase(), "value" to it.value.toString()) }), "closed" to value.closed, "ignoredProperties" to jsonArray(value.ignoredProperties.map { it.value }), "severity" to value.severity.name.lowercase(), "message" to value.message)

...
export default function StagingPanel({ projectId, sourceId }: { projectId: string; sourceId: string }) {
  ...
  return (
    <section className="staging-panel" aria-labelledby="staging-heading">
      ...
      {proposal ? <div className="proposal-summary" aria-live="polite"><strong>Proposal {proposal.status.toLowerCase()}</strong>{proposal.message ? <p>{proposal.message}</p> : null}{proposal.validationMessages.map((message) => <p key={message} role="alert">{message}</p>)}{proposal.diff.length ? <ul>{proposal.diff.map((entry) => <li key={`${entry.kind}-${entry.description}`}>{entry.description}</li>)}</ul> : null}</div> : null}
    </section>
  );
}

...
The approved post-completion amendment replaces general dependency layering with a deterministic hierarchy-first class tree and adds `Hierarchy`, `Focus`, and `Full map` presentation modes. It is limited to existing asserted graph DTOs and temporary React state. No semantic contract, server route, persistence, edit behavior, module, or dependency is added.

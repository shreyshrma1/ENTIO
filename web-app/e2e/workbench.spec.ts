...
test("completes the browser workbench and provider model journey through reviewable staging", async ({ page }) => {
  ...
  await expect(page.getByRole("tabpanel", { name: "Settings workspace" })).toBeVisible();
  ...
  const proposalReview = page.getByRole("dialog", { name: "View Details" });
  await expect(proposalReview.getByRole("button", { name: "Remove" }).first()).toBeEnabled();
  ...
  await proposalReview.getByRole("button", { name: "Accept" }).click();
  ...
  await expect(page.getByText("Updating staged changes...", { exact: true })).toHaveCount(0);
  await proposalReview.getByRole("button", { name: "Close", exact: true }).click();
  await expect(reviewQueue.getByText("0", { exact: true })).toBeVisible();

...
function ShaclConstraintDialog(props: ShaclConstraintDialogProps) {
  ...
  const [pathDomainKind, setPathDomainKind] = useState<"ObjectProperty" | "DatatypeProperty">("ObjectProperty");
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) props.onClose(); }}>
    <section className="edit-dialog shacl-constraint-dialog" role="dialog" aria-modal="true" aria-label={editing ? shapeName || "Constraint edit" : "Add constraint"}>
      ...
      {editing ? <form onSubmit={updateConstraint}>
        ...
        {props.entity.kind.toLowerCase() === "class" ? <ReadOnlyShaclContext label="Target class" value={props.entity.label} /> : <SemanticClassPicker projectId={props.projectId} id="shacl-target-class" label="Target class" selected={props.selectedTarget} onChange={props.onTargetChange} multiple={false} domainRecommendation={{ operationKind: "ShaclTargetClass", requestedKind: "Class", targetSourceId: props.entity.sourceId }} />}
        {props.entity.kind.toLowerCase() === "class" ? <><label>FIBO property kind<select value={pathDomainKind} onChange={(event) => setPathDomainKind(event.target.value as "ObjectProperty" | "DatatypeProperty")}><option value="ObjectProperty">Object property</option><option value="DatatypeProperty">Datatype property</option></select></label><SemanticEntityPicker projectId={props.projectId} id="shacl-property-path" label="Property path" selected={props.selectedPath} onChange={props.onPathChange} accepts={acceptsProperty} placeholder="Search properties" help="Choose a direct property path." multiple={false} selectedValueInInput selectionPresentation="hidden" includeStaged={false} domainRecommendation={{ operationKind: "ShaclPropertyPath", requestedKind: pathDomainKind, currentEntityIri: props.entity.iri, targetSourceId: props.entity.sourceId }} /></> : <><ReadOnlyShaclContext label="Property path" value={props.entity.label} />{domainKindForEntityKind(props.entity.kind) ? <DomainRecommendationPanel projectId={props.projectId} draftLabel={props.entity.label} intent={{ operationKind: "ShaclPropertyPath", requestedKind: domainKindForEntityKind(props.entity.kind)!, currentEntityIri: props.entity.iri, targetSourceId: props.entity.sourceId }} localTarget={{ iri: props.entity.iri, sourceId: props.entity.sourceId }} compact /> : null}</>}
        <div className="shacl-dialog-grid"><label>Constraint<select value={props.constraintKind} onChange={(event) => { const next = event.target.value; props.onConstraintKindChange(next); if (next === "datatype" && !props.constraintValue.startsWith(XSD_NAMESPACE)) props.onConstraintValueChange(XSD_STRING); }}>{SHACL_CONSTRAINTS.map((constraint) => <option key={constraint.value} value={constraint.value}>{constraint.label}</option>)}</select></label><label>Constraint value{props.constraintKind === "datatype" ? <select value={props.constraintValue} onChange={(event) => props.onConstraintValueChange(event.target.value)}>{LITERAL_DATATYPES.map((datatype) => <option key={datatype.iri} value={datatype.iri}>{datatype.label}</option>)}</select> : <input value={props.constraintValue} onChange={(event) => props.onConstraintValueChange(event.target.value)} placeholder="1" />}</label><label>Severity<select value={props.severity} onChange={(event) => props.onSeverityChange(event.target.value)}><option>Violation</option><option>Warning</option><option>Info</option></select></label></div>
        {props.constraintKind === "class" ? <DomainRecommendationPanel projectId={props.projectId} draftLabel={props.constraintValue} intent={{ operationKind: "ShaclClassOrDatatypeConstraint", requestedKind: "Class", currentEntityIri: props.entity.iri, targetSourceId: props.entity.sourceId }} compact /> : null}
